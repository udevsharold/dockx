...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch(indexPath.section) {
        case 0: {
            ...
            label = [DockXHelper localizedStringForActionNamed:[DockXHelper actionNameFromArray:self.fullOrder[0] atIndex:indexPath.row] shortName:NO bundle:tweakBundle];
           // label = [DockXHelper labelFromArray:self.fullOrder[0] atIndex:indexPath.row];
            image = [DockXHelper imageFromArray:self.fullOrder[0] atIndex:indexPath.row withSystemColor:YES completion:^(BOOL thirteen, BOOL customPath){
                isThirteen = thirteen;
        ...
        case 1: {
            ...
            label = [DockXHelper localizedStringForActionNamed:[DockXHelper actionNameFromArray:self.fullOrder[1] atIndex:indexPath.row] shortName:NO bundle:tweakBundle];

            //label = [DockXHelper labelFromArray:self.fullOrder[1] atIndex:indexPath.row];
            image = [DockXHelper imageFromArray:self.fullOrder[1] atIndex:indexPath.row withSystemColor:YES completion:^(BOOL thirteen, BOOL customPath){
                isThirteen = thirteen;
                isCustomImagePath = customPath;
                dispatch_semaphore_signal(smp);
            }];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [[PrefsManager sharedInstance] writePrefs:self.prefs];
...
- (void)viewDidLoad {
    ...
    [tweakBundle load];
    [super viewDidLoad];
    self.prefs = [[[PrefsManager sharedInstance] readPrefs] mutableCopy];

...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section{
    ...
    switch (section) {
        ...
        case 2:
            ...
            if (tranzloInstalled){
                footerTextForSectionOne = [footerTextForSectionOne stringByAppendingString:@"\n"];
                footerTextForSectionOne = [footerTextForSectionOne stringByAppendingString:LOCALIZED(@"FOOTER_TEXT_FOR_DEFAULT_LONG_PRESS_GESTURES_TRANZLO")];
            }
...
- (void)updateOrder:(BOOL)reset{
    ...
    if (reset){
        ...
        for (NSString *cacheFile in [fm contentsOfDirectoryAtPath:DockXCachePath error:nil]) {
            [fm removeItemAtPath:[NSString stringWithFormat:@"%@/%@", DockXCachePath, cacheFile] error:nil];
        }
    }
